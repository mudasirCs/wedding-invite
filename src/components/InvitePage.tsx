...
function HostsList() {
  ...
  return (
    <RevealSection
      id="hosts"
      className={`relative overflow-visible px-4 ${isRtl ? 'invite-section' : 'py-14'}`}
    >
      <RevealImg
        src="/media/floral-2.png"
        alt=""
        aria-hidden
        className="pointer-events-none absolute -top-4 -left-3 z-0 w-28 select-none opacity-70 sm:w-32"
        loading="lazy"
        decoding="async"
        from="fade"
      />
      <RevealImg
        ...
        alt=""
        aria-hidden
        className="pointer-events-none absolute -right-3 -bottom-2 z-0 w-28 scale-x-[-1] select-none opacity-70 sm:w-32"
        loading="lazy"
        decoding="async"
        from="fade"
        delay={0.1}
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-md flex-col items-center gap-6">
        <div className="text-center">
          <RevealImg
            src="/media/icons/icon-ourstory.png"
            alt=""
          ...
          <h2
            className={`font-headline font-normal text-[#831843] ${
              isRtl ? 'invite-title mx-auto max-w-[16rem] px-2' : 'text-4xl italic sm:text-[2.75rem]'
            }`}
            style={
              isRtl
                ? undefined
          ...
          >
            {t.hostsAwaitingHeading}
          </h2>
          <div className="mt-4 flex items-center justify-center gap-2.5" aria-hidden>
            <span className="h-[1.5px] w-10 bg-[#831843]/35" />
            <span className="h-1.5 w-1.5 rotate-45 bg-[#831843]/55" />
            <span className="h-[1.5px] w-10 bg-[#831843]/35" />
          </div>
        </div>

        <Reveal delay={0.06} className="w-full">
          <div className="invite-glass hosts-card mx-auto w-full max-w-[19rem] rounded-2xl px-5 py-6 text-center">
            <ul className="m-0 list-none space-y-2.5 p-0">
              {t.hostsAwaitingNames.map((name) => (
                <li
                  key={name}
                  className={`font-formal text-ink ${
                    isRtl ? 'invite-body leading-snug' : 'text-[15px] tracking-[0.04em]'
                  }`}
                >
                  {name}
                </li>
        ...
        <Reveal delay={0.18} className="w-full">
          <div className="invite-glass hosts-card relative mx-auto w-full max-w-[19rem] overflow-hidden rounded-2xl px-5 py-6 text-center">
            <img
              src="/media/rsvp-floral-corner.webp"
              alt=""
              aria-hidden
              className="pointer-events-none absolute -right-5 -bottom-6 w-28 opacity-45"
              loading="lazy"
              decoding="async"
            />
            <h3
              className={`relative z-10 font-formal font-semibold text-[#831843] ${
                isRtl
                  ? 'invite-subtitle mx-auto max-w-[14rem]'
                  : 'text-sm tracking-[0.14em] uppercase'
              }`}
            >
              {t.hostsComplimentsHeading}
            </h3>
            <div className="relative z-10 mt-3 mb-4 flex items-center justify-center gap-2" aria-hidden>
              <span className="h-px w-8 bg-[#831843]/30" />
              <span className="h-1 w-1 rotate-45 bg-[#831843]/45" />
              <span className="h-px w-8 bg-[#831843]/30" />
            </div>
            <ul className="relative z-10 m-0 list-none space-y-2.5 p-0">
              {t.hostsComplimentsNames.map((name) => (
                <li
                  key={name}
                  className={`font-formal text-ink ${
                    isRtl ? 'invite-body leading-snug' : 'text-[15px] tracking-[0.04em]'
            ...
            <a
              href={`tel:${t.hostsComplimentsPhone.replace(/\D/g, '')}`}
              className="font-formal relative z-10 mt-5 inline-block text-[15px] font-semibold tracking-[0.06em] text-[#831843] underline-offset-4 hover:underline"
              dir="ltr"
              lang="en"
            >
              {t.hostsComplimentsPhone}
            </a>
          </div>
        </Reveal>
      </div>
